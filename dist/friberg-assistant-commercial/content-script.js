(() => {
  ...
  const isLocalLiveFixture = () => false;
  ...
  function updateLiveOverlay(detail) {
    ...
    state.overlay.update({
      ...
      randomEnabled: Boolean(live.listening && !live.feedbackPaused && input && firstGuessAvailable && !live.submissionInFlight),
  ...
  function synchronizeLiveFeedback() {
    ...
    const rows = FribergLiveDomAdapter.feedbackRows(live.activeBoard);
    if (!rows.length && live.session.history.length) {
      createLiveSession('检测到棋盘反馈已清空；按新局重新等待反馈。', { resetSubmitCooldown: true });
      updateLiveOverlay();
      return;
    }
    live.feedbackPaused = false;
  ...
  function bindLiveBoard(candidate, source) {
    ...
    live.boardObserver.observe(live.activeBoard, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'data-feedback', 'data-state', 'aria-label'] });
  ...
  function attemptQueuedLiveSubmit() {
    ...
    const cooldownRemaining = live.lastSubmitAt + SUBMIT_COOLDOWN_MS - Date.now();
    if (cooldownRemaining > 0) {
      live.status = '等待网页提交冷却';
      live.detail = `已排队 ${player.nick}；约 ${Math.max(1, Math.ceil(cooldownRemaining / 100) / 10)} 秒后自动提交。`;
      live.error = '';
      scheduleLiveSubmitRetry(Math.min(SUBMIT_RETRY_INTERVAL_MS, cooldownRemaining));
      updateLiveOverlay();
      return;
    }
  ...
  function startLiveAssistant() {
    ...
    state.live.pageObserver.observe(document.documentElement, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'disabled', 'data-state', 'data-feedback', 'aria-disabled', 'aria-label'] });
