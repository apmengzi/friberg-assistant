...
风险：仓库级配置可能让后续自动化助手在本机以过宽权限运行。该文件不属于插件运行所需内容。

处理：开发基线 PR 已删除该文件，并忽略 `.codex/`。

### 2. 运行时浏览器快照被提交

首次导入包含 `.playwright-mcp/page-*.yml`。此类文件未来可能包含房间文本、用户输入或临时页面状态。

处理原则：

- `.gitignore` 已忽略该目录；
- 清理 PR 删除已跟踪快照；
- 需要保留的 DOM fixture 必须脱敏后移入 `tests/fixtures/`。
...
部分安装说明曾包含开发机专用的用户目录绝对路径。

风险较低，但会暴露本地目录结构，并让安装说明不可移植。

处理：统一改成项目相对路径或 `<项目根目录>` 占位符。
...
- 将在线旧构建明确标记为 legacy / development only；
- 构建审计继续禁止把 localhost 版本当作销售包；
- 唯一离线商业候选交付目录为 `dist/friberg-assistant-commercial/`。

## 中优先级结构问题

### 6. 核心文件存在多份复制

根目录、`extension/`、`dist/` 和旧备份中都有：

- `solver.js`
- `automation-core.js`
- `live-dom-adapter.js`

打包脚本表明根目录版本是主要来源，再复制到 `extension/` 和 `dist/`。长期手改任意副本容易产生漂移。
...
后续应将其缩减为必要的脱敏 fixture、文件哈希、来源说明与关键片段，不作为长期正式源码。

### 8. 备份目录与正式源码并存

`artifacts/backups/` 保存旧扩展副本。Git 已经承担版本历史职责，继续在仓库中保存完整副本会增加误改和搜索噪声。

后续确认基准标签后删除完整备份，使用 Git tag/branch 代替。
...
1. 删除已跟踪运行时快照、缓存和日志；
2. 将本地绝对路径改为可移植说明；
3. 标记或清除 legacy localhost 商业构建；
4. 缩减旧备份和下载的站点资源；
5. 统一源码/生成物边界；
6. 审计扩展权限；
7. 保持仓库 Private，直到许可证和第三方资源边界另行确认。
